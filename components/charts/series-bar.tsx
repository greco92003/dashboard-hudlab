...
function computeSeriesBarLayout(input: {
  ...
  // Barras partem da linha do zero (não do rodapé) para aceitar valores
  // negativos — o fluxo de caixa desenha saídas abaixo de zero.
  const valuePos = yScale(value) ?? innerHeight;
  const zeroPos = Math.min(innerHeight, Math.max(0, yScale(0) ?? innerHeight));
  return {
    barLeft: xCenter - groupWidth / 2 + seriesIndex * (barWidth + gap),
    barHeight: Math.abs(zeroPos - valuePos),
    effectiveRadius: radius,
    valueY: Math.min(valuePos, zeroPos),
  };
}
