...
export default function VariableCostsPage() {
  ...
  const fetchVariableCosts = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error("Error fetching variable costs:", result.error);
      }
    } catch (error) {
  ...
  const handleAddCost = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert("Erro ao adicionar custo variável: " + result.error);
      }
    } catch (error) {
      console.error("Error adding variable cost:", error);
      alert("Erro ao adicionar custo variável");
    }
  };

  // Delete cost
  const handleDelete = async (
    costToDelete: VariableCost,
    mode: "single" | "future"
  ) => {
    try {
      ...
      const response = await fetch(`/api/variable-costs?${params.toString()}`, {
        method: "DELETE",
      });
      ...
      if (response.ok) {
        ...
        console.error("Error deleting variable cost:", result.error);
        alert("Erro ao deletar custo variável: " + result.error);
      }
    } catch (error) {
      console.error("Error deleting variable cost:", error);
      alert("Erro ao deletar custo variável");
    }
  };
  ...
  const performEdit = async (mode: "single" | "all") => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert("Erro ao atualizar custo variável: " + result.error);
      }
    } catch (error) {
      console.error("Error updating variable cost:", error);
      alert("Erro ao atualizar custo variável");
    }
  };
  ...
  return (
    <div className="flex flex-1 flex-col gap-4">
      <h1 className="text-xl sm:text-2xl font-bold">Custos Variáveis</h1>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild>
            <Button className="w-full sm:w-auto">
              ...
              <span className="hidden sm:inline">Adicionar Custo Variável</span>
              <span className="sm:hidden">Adicionar</span>
            </Button>
          </DialogTrigger>
          <DialogContent onClick={handleDialogInteraction}>
            <DialogHeader>
              <DialogTitle>Adicionar Novo Custo Variável</DialogTitle>
            </DialogHeader>
            ...
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  placeholder="Descrição do custo variável"
                />
              </div>
