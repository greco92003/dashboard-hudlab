...
import { ProductionOrderButton } from "@/components/erp/production-order-button";
  ...
  return <>
    <Card>
      ...
      <CardContent className="space-y-5">
        ...
        <div className="flex flex-col justify-between gap-3 rounded-lg bg-muted/30 p-4 sm:flex-row sm:items-center"><div><p className="text-sm font-medium">Total: {total.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</p>{result && <p className="text-xs text-emerald-600">Pedido {result.number || result.id} confirmado no Tiny.</p>}</div>{result ? <OrderDocumentButtons baseHref={`/api/erp/tiny/orders/${result.id}`} size="default" talaoLabel="Gerar talão" romaneioLabel="Gerar romaneio" className="lg:flex-wrap"><ProductionOrderButton orderId={result.id} size="default" /><ProductionLabelsButton href={`/api/erp/tiny/orders/${result.id}/etiquetas`} size="default" label="Imprimir etiquetas" /></OrderDocumentButtons> : <Button disabled={problems.length > 0 || saving} onClick={() => setConfirmOpen(true)}>{saving ? <Loader2 className="animate-spin" /> : <ReceiptText />}{saving ? "Criando…" : "Criar pedido de venda"}</Button>}</div>
      </CardContent>
    </Card>
