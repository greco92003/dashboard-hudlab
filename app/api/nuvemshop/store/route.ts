...
export async function GET(request: NextRequest) {
  try {
    ...
    const endpoint = `/store${queryString ? `?${queryString}` : ""}`;

    // Fetch store information from Nuvemshop
    const store = await fetchNuvemshopAPI(endpoint);
    ...
    console.error("Error fetching Nuvemshop store information:", error);
    return NextResponse.json(
      {
