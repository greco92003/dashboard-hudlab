// =====================================================
// WEBHOOK ESPECÍFICO: ORDER UPDATED
// =====================================================
// Endpoint específico para webhooks de pedidos atualizados
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`📦 Processing order updated: ${orderId} from store ${storeId}`);
    ...
    // Processar webhook
    const result = await processor.processWebhook("order/updated", payload, webhookLog.id);

    const totalTime = Date.now() - startTime;

    if (result.success) {
      console.log(`✅ Order updated webhook processed in ${totalTime}ms`);
      ...
      console.error(`❌ Order updated webhook failed: ${result.error_message}`);
