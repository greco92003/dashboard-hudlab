// =====================================================
// WEBHOOK ESPECÍFICO: COUPON UPDATED
// =====================================================
// Endpoint específico para webhooks de cupons atualizados
...
export async function POST(request: NextRequest) {
  ...
  try {
    console.log("🔔 Coupon updated webhook received");
    ...
    console.log(`🔄 Processing coupon updated: ${couponId} from store ${storeId}`);
    ...
    // 3. Processar atualização do cupom usando o processor
    const processor = new NuvemshopWebhookProcessor();
    const result = await processor.processWebhook("coupon/updated", payload, webhookLog.id);

    const totalTime = Date.now() - startTime;

    if (result.success) {
      console.log(`✅ Coupon updated webhook processed in ${totalTime}ms`);
      
      return NextResponse.json({
        success: true,
        message: "Coupon updated webhook processed",
        coupon_id: couponId,
        processing_time_ms: totalTime,
      });
    } else {
      console.error(`❌ Coupon updated webhook failed: ${result.error_message}`);
    ...
    const totalTime = Date.now() - startTime;
    console.error("Coupon updated webhook error:", error);
