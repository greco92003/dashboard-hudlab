...
-- Policy for authenticated users to read tracking data
CREATE POLICY "Allow authenticated users to read deals tracking" ON deals_processed_tracking
  FOR SELECT TO authenticated USING (true);

-- Policy for service role to manage tracking data
CREATE POLICY "Allow service role to manage deals tracking" ON deals_processed_tracking
  FOR ALL TO service_role USING (true);

-- Create view for monitoring and analytics with proper security
CREATE OR REPLACE VIEW deals_tracking_stats
WITH (security_invoker = true) AS
SELECT
  COUNT(*) as total_tracked_deals,
  COUNT(*) FILTER (WHERE has_custom_field_5 = true) as deals_with_field_5,
  COUNT(*) FILTER (WHERE has_any_target_fields = true) as deals_with_any_target_fields,
  COUNT(*) FILTER (WHERE target_fields_found IS NOT NULL AND array_length(target_fields_found, 1) > 0) as deals_with_multiple_fields,
  MAX(last_checked_at) as last_sync_time,
  MIN(last_checked_at) as first_sync_time,
  COUNT(DISTINCT sync_batch_id) as total_sync_batches,
  AVG(processing_time_ms) as avg_processing_time_ms,
  MAX(processing_time_ms) as max_processing_time_ms
FROM deals_processed_tracking;

-- Grant permissions for the view (only to authenticated users and service role)
GRANT SELECT ON deals_tracking_stats TO authenticated, service_role;

-- Add RLS policy for the view to ensure it respects the table's RLS
CREATE POLICY "deals_tracking_stats_policy" ON deals_processed_tracking
