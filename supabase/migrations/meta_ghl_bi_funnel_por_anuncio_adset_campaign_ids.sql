-- ============================================================
-- Ajuste 2026-07-27 (aplicado no Dashboard-v2 via MCP, com DROP+CREATE
-- pois mudou o tipo de retorno da função)
--
-- get_funnel_por_anuncio passa a expor campaign_id, adset_id e adset_name
-- (já sincronizados em meta_insights_daily, só não estavam expostos) --
-- permite a tabela "Funil por anúncio" (Anúncios) agrupar por
-- Campanha/Conjunto de Anúncios/Anúncio, agregando client-side a partir
-- do nível mais granular (ad_id) em vez de duplicar a lógica da função
-- pra cada nível.
--
-- Ver app/meta-marketing-ghl/components/anuncios.tsx: agruparPorNivel()
-- soma os totais brutos por campaign_id/adset_id e recalcula toda métrica
-- derivada (R$/lead, ROAS etc.) a partir dos totais agregados -- nunca
-- soma ou faz média de uma métrica que já é uma razão.
-- ============================================================

DROP FUNCTION IF EXISTS public.get_funnel_por_anuncio(date, date);

CREATE FUNCTION public.get_funnel_por_anuncio(p_inicio date, p_fim date)
