-- ============================================================
-- Ajuste 2026-07-24 (parte 7b, aplicado no Dashboard-v2 via MCP)
--
-- Generaliza a definição de "órfão" pra leads_ghl. Achado residual
-- (LorinhoFut: 3 leads, 4 orçamentos mesmo após a parte 7): existem
-- contatos com oportunidade JÁ sincronizada, mas cujo ad_id (gerado de
-- ghl_contacts.utm_content) é NULL ou diferente do ad_id que o payload do
-- webhook capturou no momento do evento -- um desalinhamento de
-- atribuição (utm_content do contato pode ter sido sobrescrito/ficou
-- vazio depois), não falta de sincronização.
--
-- Fix: "órfão" (soma no leads_ghl do anúncio X) agora significa "não
-- existe em `opp` PARA ESSE ad_id especificamente" (contact_id + ad_id),
-- em vez de só "não tem nenhuma oportunidade em lugar nenhum". Cobre os
-- dois casos: sem oportunidade nenhuma (fusão de contato) E oportunidade
-- existente mas atribuída a outro anúncio/nenhum (deriva de UTM).
--
-- Verificado: zero anúncios com orçamentos/mockups/negociações > leads_ghl
-- após esta correção.
-- ============================================================
