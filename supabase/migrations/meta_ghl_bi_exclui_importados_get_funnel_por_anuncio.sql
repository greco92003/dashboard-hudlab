-- ============================================================
-- Ajuste 2026-07-25 (aplicado no Dashboard-v2 via MCP, em duas iterações
-- consolidadas aqui: meta_ghl_bi_exclui_eventos_importados depois
-- meta_ghl_bi_exclui_importados_view)
--
-- get_funnel_por_anuncio passa a excluir qualquer contato presente em
-- v_contatos_importados (ver migration v_contatos_importados.sql) -- tanto
-- no braço pipeline (`opp`) quanto no braço webhook (`webhook_eventos`).
--
-- Sem isso, o fix de "leads órfãos" (meta_ghl_bi_leads_ghl_generaliza_orfao,
-- 2026-07-24) estava contando 77 contatos de um lote de importação em
-- massa (231 contatos, evento "negociofechado" disparado num intervalo de
-- 9 minutos em 24/07, com tags de negócios antigos já fechados) como
-- "leads novos" no período de 7 dias -- inflando leads_ghl/orcamentos/etc.
-- pra vários ad_ids que na verdade não têm nenhum investimento real no
-- Meta associado a essa atividade.
-- ============================================================
