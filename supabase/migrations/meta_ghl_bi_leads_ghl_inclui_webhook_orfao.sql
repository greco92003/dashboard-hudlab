-- ============================================================
-- Ajuste 2026-07-24 (parte 7, aplicado no Dashboard-v2 via MCP)
--
-- leads_ghl (por anúncio, get_funnel_por_anuncio) passa a incluir contatos
-- atribuídos ao anúncio via webhook que ainda NÃO têm oportunidade
-- sincronizada em ghl_opportunities -- evita o mesmo problema de fusão de
-- contato que já resolvemos pro "Lead" geral, mas agora entre Leads e
-- Orçamentos/Mockups/Negociações NA MESMA TABELA: Orçamentos/Mockups/
-- Negociações (webhook, ad_id do payload) não dependem de oportunidade
-- sincronizada, então um contato pode aparecer em "Orçamentos" sem
-- aparecer em "Leads" (só pipeline) -- gerando o absurdo visual "mais
-- orçamentos que leads" (ex.: JudoNoticia: 4 leads, 6 orçamentos).
--
-- Fix: soma ao leads_ghl (pipeline) os contatos "órfãos" -- atribuídos ao
-- anúncio via webhook, SEM oportunidade sincronizada -- garantindo
-- leads_ghl >= orcamentos >= mockups >= negociacoes sempre. Superado pela
-- migration seguinte (meta_ghl_bi_leads_ghl_generaliza_orfao), que
-- generaliza a definição de "órfão" pra cobrir também o caso de
-- oportunidade já sincronizada mas atribuída a ad_id diferente/nenhum.
-- ============================================================
...
AS $function$
  ...
  webhook_agg as (
    select
      ...
      count(*) filter (where opportunity_id is null) as leads_orfaos_webhook
