...
async function testGoogleSheetsAPI() {
  ...
  try {
    ...
    console.log(`   📧 Client Email: ${clientEmail ? '✅ Configurado' : '❌ Não configurado'}`);
    console.log(`   🆔 Project ID: ${projectId ? '✅ Configurado' : '❌ Não configurado'}`);
    console.log(`   🔑 Private Key: ${privateKey ? '✅ Configurado' : '❌ Não configurado'}`);
    console.log(`   📊 Spreadsheet ID: ${spreadsheetId ? '✅ Configurado' : '❌ Não configurado'}\n`);
    ...
    console.log(`   📄 Range: Mockups Feitos!A1:Z10`);
    ...
    if (values.length > 0) {
      ...
      if (values.length > 1) {
        ...
        values.slice(1, Math.min(4, values.length)).forEach((row, index) => {
          console.log(`      Linha ${index + 2}: ${row.slice(0, 3).join(' | ')}...`);
        });
      }
    } else {
    ...
    if (error.message.includes('403')) {
      ...
      console.log('      2. Planilha não foi compartilhada com o service account');
      console.log('      3. APIs não estão habilitadas no Google Cloud');
      console.log('\n💡 SOLUÇÕES:');
      console.log('   1. Compartilhe a planilha com:', process.env.GOOGLE_SHEETS_CLIENT_EMAIL);
